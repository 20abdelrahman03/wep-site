...
export function detectInApp(ua, headers) {
  ...
  if (/instagram \d|instagram\/\d/.test(ual) || xrwl.includes('com.instagram.android')) {
    app = 'Instagram';
  } else if (/fban|fbav|fb_iab/.test(ual) || xrwl.includes('com.facebook')) {
    app = 'Facebook';
  } else if (/tiktok/.test(ual) || xrwl.includes('com.zhiliaoapp.musically')) {
    app = 'TikTok';
  } else if (/snapchat/.test(ual)) {
    app = 'Snapchat';
  } else if (/twitter/.test(ual)) {
    app = 'X/Twitter';
  } else if (/line\//.test(ual)) {
    app = 'LINE';
  } else if (/whatsapp/.test(ual)) {
    app = 'WhatsApp';
  } else if (/micromessenger/.test(ual)) {
    app = 'WeChat';
  } else if (/pinterest/.test(ual)) {
    app = 'Pinterest';
  } else if (/reddit\/\d/.test(ual)) {
    app = 'Reddit';
  }
  ...
  if (/;\s*wv\)/.test(ual) || (xrwl && /^com\.[a-z0-9_.]+$/i.test(xrw) && !app)) {
    isWebview = true;
    if (xrw) apps.push(xrw);
  }
  if (/iphone|ipad/.test(ual) && /safari/.test(ual) && !/chrome|crios|fxios|edg\//.test(ual) && !/macintosh/.test(ual)) {
