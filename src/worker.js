...
// Single canonical routing constants — every route below is expressed in terms
// of these, so there is exactly ONE login page and ONE dashboard page.
...
// Page guard: null when authenticated, otherwise ONE deterministic 302 to the
// login page. Never used for API routes (they must return JSON, not HTML).
async function requirePageAuth(request, env) {
  return (await isAdmin(request, env)) ? null : Response.redirect(new URL(LOGIN_PAGE, request.url), 302);
}
// API guard: null when authenticated, otherwise 401 JSON — so dashboard JS
// never receives an HTML login page or a redirect it cannot follow.
async function requireApiAuth(request, env) {
  return (await isAdmin(request, env)) ? null : jsonResponse({ ok: false }, 401);
}
...
async function runMigrations(db) {
  // Fast path: with run_worker_first the Worker runs for every request, so
  // avoid re-running ~40 idempotent ALTER/CREATE statements per request once
  // the v2 schema is in place. Only the cheap backfill check remains.
  try {
    const cols = (await db.prepare(`SELECT name FROM pragma_table_info('sessions')`).all()).results;
    const names = new Set(cols.map(c => c.name));
    if (names.has('classification') && names.has('js_beacons') && names.has('active_duration_s')) {
      await backfillLegacy(db);
      return;
    }
  } catch (e) { /* pragma unavailable — fall through to the full migration */ }
...
export default {
  async fetch(request, env) {
    ...
    let p = url.pathname;
    // ONE canonical path form: strip trailing slashes (except the root) so that
    // /dashboard/ == /dashboard and /login/ == /login. This removes the
    // slash-vs-no-slash redirect ping-pong between the Worker and the static
    // asset handler (html_handling) that produced the production redirect loop.
    ...
    try {
      ...
      /* ─────────── public pages (served from assets, NEVER guarded) ─────────── */
      // /login and /privacy are public. They are served through the normalized
      // path so the asset handler returns 200 directly (no trailing-slash 307).
      if (p === LOGIN_PAGE || p === '/privacy') {
        const assetUrl = new URL(request.url);
        assetUrl.pathname = p;
        return env.ASSETS.fetch(new Request(assetUrl, request));
      }
      // .html alias of a public page → one clean 302 to the canonical path.
      if (p === '/login.html') return Response.redirect(new URL(LOGIN_PAGE, request.url), 302);
      ...
      if (p === '/admin/login' && request.method === 'GET')
        return Response.redirect(new URL(LOGIN_PAGE, request.url), 302);
      ...
      // ROOT-CAUSE FIX: the dashboard asset is fetched through the ORIGINAL
      // normalized path (/dashboard), which the static-asset handler serves
      // with 200. The old code fetched '/dashboard.html' — with html_handling
      // auto-trailing-slash that returns 307 → /dashboard, which is worker-first,
      // which fetches '/dashboard.html' again → ERR_TOO_MANY_REDIRECTS.
      if (p === DASHBOARD_PAGE || p.startsWith(DASHBOARD_PAGE + '/')) {
        const deny = await requirePageAuth(request, env);
        if (deny) return deny;
        const assetUrl = new URL(request.url);
        assetUrl.pathname = p;
        const assetRes = await env.ASSETS.fetch(new Request(assetUrl, request));
        // Prevent CDN / browser from caching a stale dashboard HTML: the file
        // ships an auth-wall that toggles based on a live API call, so any
        // cached copy across a login boundary would show the wrong UI.
        const h = new Headers(assetRes.headers);
        h.set('Cache-Control', 'no-store, no-cache, must-revalidate');
        h.set('Pragma', 'no-cache');
        return new Response(assetRes.body, { status: assetRes.status, headers: h });
      }
      // Canonical aliases → exactly ONE redirect to /dashboard (never a loop).
      if (p === '/dashboard.html' || p === '/admin') {
        const deny = await requirePageAuth(request, env);
        if (deny) return deny;
        return Response.redirect(new URL(DASHBOARD_PAGE, request.url), 302);
      }

      /* ═══════════ admin APIs — JSON 401 when unauthenticated, never an HTML redirect ═══════════ */
      if (p.startsWith('/admin/api/')) {
        const deny = await requireApiAuth(request, env);
        if (deny) return deny;
        ...
        if (p === '/admin/api/export.json') {
          // Full-fidelity export. Honours the active filters when present so an
          // admin can drill down + export, but the default (no filters) is a
          // complete dump of every safe field the schema stores. Internal/test
          // sessions live in their OWN top-level bucket so they never pollute
          // the primary analytics tables. NO secrets, cookies, or raw IPs are
          // ever included — only hashed IPs (already stored that way).
          const scope = (f && Object.keys(f).length) ? 'filtered' : 'all';
          const [
            summaryRes, traffic, devices, browsers, oses, vendors, inappBreak,
            classifications, overTime, sess, vis, evts, reqs, internalSess, live
          ] = await Promise.all([
            db.prepare(`SELECT
              (SELECT COUNT(*) FROM visitors) visitors_all,
              (SELECT COUNT(DISTINCT visitor_id) FROM sessions
                 WHERE COALESCE(classification,'unknown') != 'internal') visitors_external,
              (SELECT COUNT(*) FROM sessions WHERE COALESCE(is_internal,0)=0) sessions_external,
              ...
              (SELECT SUM(COALESCE(pageviews,1)) FROM sessions WHERE COALESCE(is_internal,0)=0) pageviews_external,
              (SELECT COUNT(*) FROM events) events_total,
              (SELECT ROUND(AVG(CASE WHEN duration_s >= 5 AND duration_s < 3600 THEN duration_s END))
                 FROM sessions WHERE COALESCE(is_internal,0)=0) avg_duration_s_external
            `).first(),
            db.prepare(`SELECT COALESCE(s.source,'direct') k, COUNT(*) sessions,
                SUM(CASE WHEN s.classification='human_likely' THEN 1 ELSE 0 END) human,
                SUM(CASE WHEN s.classification='bot' THEN 1 ELSE 0 END) bot,
                AVG(CASE WHEN s.duration_s >= 5 AND s.duration_s < 3600 THEN s.duration_s END) avg_duration_s
              FROM sessions s ${where} GROUP BY COALESCE(s.source,'direct') ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.device_type,'unknown') k, COALESCE(s.device_vendor,'unknown') vendor,
                COALESCE(s.device_model,'unknown') model, COUNT(*) sessions
              FROM sessions s ${where} GROUP BY k, vendor, model ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.browser,'unknown') k, COUNT(*) sessions
              FROM sessions s ${where} GROUP BY k ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.os,'unknown') k, COALESCE(s.os_version,'') version, COUNT(*) sessions
              FROM sessions s ${where} GROUP BY k, version ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.device_vendor,'unknown') k, COUNT(*) sessions
              FROM sessions s ${where} GROUP BY k ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.in_app_app,'none') k, COUNT(*) sessions
              FROM sessions s ${where} AND s.is_inapp = 1 GROUP BY k ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT COALESCE(s.classification,'unknown') k, COUNT(*) sessions,
                AVG(s.confidence) avg_confidence
              FROM sessions s ${where} GROUP BY k ORDER BY sessions DESC`).bind(...vals).all(),
            db.prepare(`SELECT substr(s.started_at, 1, 13) hour, COUNT(*) sessions,
                SUM(CASE WHEN s.classification='human_likely' THEN 1 ELSE 0 END) human,
                SUM(CASE WHEN s.classification='bot' THEN 1 ELSE 0 END) bot,
                SUM(COALESCE(s.pageviews,1)) pageviews
              FROM sessions s ${where} GROUP BY hour ORDER BY hour`).bind(...vals).all(),
            db.prepare(`SELECT s.*, v.visit_count, v.first_seen visitor_first_seen
              FROM sessions s JOIN visitors v ON v.id = s.visitor_id ${where}
              ORDER BY s.started_at DESC LIMIT 5000`).bind(...vals).all(),
            db.prepare(`SELECT id, first_seen, last_seen, visit_count, classification_summary
              FROM visitors ORDER BY last_seen DESC LIMIT 2000`).all(),
            db.prepare(`SELECT visitor_id, session_id, ts, type, path, referrer, referrer_source,
              is_ig_inapp, identity_mode, extra FROM events
              ORDER BY id DESC LIMIT 10000`).all(),
            db.prepare(`SELECT ts, visitor_id, method, path, ua, is_ig_inapp, referrer
              FROM request_logs ORDER BY id DESC LIMIT 1000`).all(),
            db.prepare(`SELECT s.*, v.visit_count FROM sessions s
              JOIN visitors v ON v.id = s.visitor_id
              WHERE s.classification = 'internal' OR COALESCE(s.is_internal,0) = 1
              ORDER BY s.started_at DESC LIMIT 500`).all(),
            db.prepare(`SELECT s.id, s.visitor_id, s.last_activity_at, s.started_at,
                s.duration_s, s.source, s.device_type, s.device_vendor, s.device_model,
                s.os, s.browser, s.classification, s.confidence, s.is_inapp, s.in_app_app,
                s.pageviews,
                (SELECT e.path FROM events e WHERE e.session_id = s.id
                   AND e.type = 'pageview' ORDER BY e.id DESC LIMIT 1) current_path
              FROM sessions s
              ...
              .bind(new Date(Date.now() - LIVE_WINDOW_S * 1000).toISOString()).all(),
          ]);
          const data = {
            export_metadata: {
              exported_at: nowIso(),
              ...
              schema_notes: {
                privacy: 'No PII. Visitor IDs are random hex. IPs stored only as HMAC-SHA256 hashes.',
                terminology: {
                  visits: 'Every page load counted (includes bots, previews, refreshes).',
                  human_likely_visits: 'Sessions classified as HUMAN_LIKELY by the weighted classifier.',
                  bot_visits: 'Sessions classified as BOT (crawler UA, no JS, extreme rate, etc.).',
                  unknown_visits: 'Insufficient evidence in either direction — not claimed as human.',
                  suspicious_visits: 'Automation-shape patterns (refresh storms, burst sessions).',
                  internal_test_traffic: 'YOUR own admin/test traffic — excluded from every primary total.',
                },
              },
            },
            summary: summaryRes,
            traffic_sources: traffic.results,
            devices: devices.results,
            browsers: browsers.results,
            operating_systems: oses.results,
            device_vendors: vendors.results,
            in_app_breakdown: inappBreak.results,
            classifications: classifications.results,
            traffic_over_time: overTime.results,
            visitors: vis.results,
            sessions: sess.results,
            events: evts.results,
            request_logs: reqs.results,
            internal_test_traffic: internalSess.results,
            live: live.results,
          };
          return new Response(JSON.stringify(data, null, 2), {
            ...
            headers: {
              ...
              'Content-Disposition': `attachment; filename="alucard-analytics-full-${nowIso().slice(0, 10)}.json"`,
      ...
      /* ─────────── any other /admin path: protected, canonicalize to /dashboard ─────────── */
      ...
      /* ─────────── static assets (served through the single router) ─────────── */
